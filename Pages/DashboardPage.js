...
exports.DashboardPage = class DashboardPage extends Page {
    ...
    console.log(`fetched Dividend detail for : ${stockDetails.data.symbol_name}  successfully dividend`);
    ...
    async generateExcelReport() {
        ...
        try {
            ...
            for (const jsonFile of jsonFiles) {
                ...
                console.log(`Checking for corresponding dividend file: '${dividendFilePath}'`);
    
                const dividendFileExists = await fs.access(dividendFilePath).then(() => true).catch(() => false);
                if (dividendFileExists) {
                    console.log(`Dividend file exists: '${dividendFilePath}'`);
                } else {
                    console.log(`No corresponding dividend file found for: '${jsonFilePath}'`);
                }
    
                // Call addJsonToExcel with the stock file path and the dividend file path if it exists
                await addJsonToExcel(outputFilePath, jsonFilePath, dividendFileExists ? dividendFilePath : null);
            }
