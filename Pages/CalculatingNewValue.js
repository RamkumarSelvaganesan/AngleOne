...
async function generateNewDataForStock(transactionDetails, stockDetails) {
  ...
  transactionData.forEach((transaction) => {
    ...
    transactionObject.one_year_projected_gain_percentage =
      oneYearProjectedGainPercentage;
    transactionObject.angleOne_commission_per_share =
      stockData.charge_per_share;
    transactionObject.total_angleOne_commission =
      stockData.charge_per_share * transaction.qty;
    ...
    totalOneYearProjectedGainPercentage +=
      oneYearProjectedGainPercentage * transaction.qty;
  });
  ...
  const averageTransactionDetails = {
    ...
    avg_one_year_projected_gain_percentage:
      totalOneYearProjectedGainPercentage / totalQty,
  };

  // Push the average_transaction_details object into the transaction_details array
  newJsonObject.transaction_details.push({
    average_transaction_details: averageTransactionDetails,
  });
  ...
  await fs.writeFile(
    filePath,
    JSON.stringify(newJsonObject, null, 2),
    (err) => {
      if (err) {
