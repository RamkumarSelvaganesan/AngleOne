...
async function generateDividendsDataForStock(dividendDetails, stockDetails) {
   ...
   // console.log("dividendDetails:", JSON.stringify(dividendDetails, null, 2));
    ...
    if (!Array.isArray(dividendsData) || dividendsData.length === 0) {
        console.log(`No dividend data available for ${stockData.symbol_name}. Skipping file creation.`);
        return null;
    }
    ...
    const filePath = `StockDetails/Dividends/${stockData.symbol_name.trim() !== "" ? stockData.symbol_name : stockData.comp_name}.json`;

    try {
        await fs.writeFile(filePath, JSON.stringify(newJsonObject, null, 2));
        console.log('Computed Dividend details for', stockData.symbol_name.trim() !== "" ? stockData.symbol_name : stockData.comp_name);
    } catch (err) {
